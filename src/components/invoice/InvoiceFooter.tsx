...
export function InvoiceFooter() {
  const itemCount = useInvoiceStore((state) => state.invoice.items.length);
  const compact = itemCount > 8;

  return (
    <footer className={`relative mt-auto overflow-hidden px-4 sm:px-10 ${compact ? "pb-24 pt-2" : "pb-24 pt-5 sm:pb-36"}`}>
      <div className="relative z-10 grid gap-8 sm:grid-cols-[1fr_240px]">
        <div className="pr-14 sm:pr-0">
          <p className={`text-xs text-zinc-600 ${compact ? "mb-4" : "mb-7"}`}>Signature</p>
          <div className="w-44 border-b-[1.5px] border-[#1a1a1a] sm:w-56" />

          <ul className={`grid text-[12px] leading-[1.35] text-[#222] ${compact ? "mt-3 gap-1" : "mt-5 gap-1.5"}`}>
