    ...
    <>
      <section className={`flex justify-end px-4 sm:px-10 ${compact ? "pb-3" : "pb-5"}`}>
        <div className="w-full sm:w-[280px]">
          <SummaryRow compact={compact} label="Subtotal" value={formatMoney(totals.subtotal, currency)} />
          <EditableSummaryRow compact={compact} label="Discount" value={invoice.discount} onChange={(value) => updateInvoice({ discount: value })} />
          <EditableSummaryRow compact={compact} label="VAT / Tax %" value={invoice.taxRate} onChange={(value) => updateInvoice({ taxRate: value })} />
          ...
          <EditableSummaryRow compact={compact} label="Advance" value={invoice.advance} onChange={(value) => updateInvoice({ advance: value })} />
        </div>
      </section>
